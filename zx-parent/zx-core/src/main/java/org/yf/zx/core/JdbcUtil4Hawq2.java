...
public class JdbcUtil4Hawq2 {

	protected Logger logger = LoggerFactory.getLogger(JdbcUtil4Hawq2.class);
	...
	public Pagination findPage(Class<?> clazz, String sqlId, final Pagination page,
			LinkedHashMap<String, Object> paraMap) {
		String sql = "SELECT\r\n" + "	t.*\r\n" + "FROM\r\n" + "	(\r\n" + "		SELECT\r\n"
				+ "			logid,\r\n" + "			to_timestamp(logtimestamp) accesstime,\r\n"
				+ "			clientip srcip,\r\n" + "			clientport srcport,\r\n"
				...
				+ "		FROM\r\n" + "			videolog_p\r\n" + "		WHERE\r\n" + "			1 = 1\r\n"
				+ "		AND logdate >= '2017-07-11 00:00:00'\r\n" + "		AND logdate <= '2017-08-31 23:59:59'\r\n"
				+ "	) T";
//		Map result = this.getDynamicStatement(sqlId, paraMap);
		String querySql = sql;//(String) result.get("SQL");
//		Object[] params = (Object[]) result.get("PARAMS");
		if (querySql == null || querySql.trim().equals("")) {
			logger.warn("IBATIS_NOT_FIND" + "(SQL ID: " + sqlId + ")");
			throw new BasalException(BasalException.ERROR, "IBATIS_NOT_FIND" + "(SQL ID: " + sqlId + ")");
		}
		try {
			// 查找总记录数
			String countSql = this.sqlCountRows(querySql);
//			System.out.println("params:\n" + Arrays.asList(params));
			int totalCount = jdbcTemplate4Hawq.queryForObject(countSql, Integer.class);
			page.setTotalCount(totalCount);

			int pageNo = page.getPageNumber();
			final int pageSize = page.getPageSize();
			final int startRow = (pageNo - 1) * pageSize;
			List list = findListBySizeAndSql(clazz, querySql, startRow, pageSize);
			page.setList(list);
		} catch (Exception e) {
